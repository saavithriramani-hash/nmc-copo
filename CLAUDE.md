...
- **Roles:** Faculty (own courses) · HoD (department; approve & lock) · Programme coordinator (POs/PSOs, matrices, programme attainment) · IQAC/Accreditation cell (institution read-all, consolidation, bundles) · Principal/Dean (read-only) · System admin (accounts, rollover, backups). Faculty see only their own courses; a locked course is editable only by HoD or above; **unlocking creates a new version, never overwrites**.
