...
| Screen | Notes |
|---|---|
| `/` | **CR-4: the dashboard** — what needs this person, sections stacked per role rather than one chosen. Counts and statuses only, never attainment (see `lib/dashboard.ts`). The course list moved to `/courses`, scoped as the Guard scopes it |
| `/courses` | Courses scoped like the Guard scopes them (own courses / whole department / the whole college for the COE) |
| `/admin/departments` | Departments and programmes — **the administrator's and, under CR-5, the Controller of Examinations' too** (`departments.manage`, added not moved). **Institution bootstrap is the administrator's alone** (`institution.create`): that record seeds the attainment parameters, and CR-1 made the Dean the only role that sets those, so it did not travel with the rest of the tree. **Rename and delete** for department/programme here, and for batch on the programme page (the COE's under CR-3): deletion refuses while anything references the row and names every blocker; never a cascade over courses, marks, rosters, roles or locked snapshots. `onDelete: Restrict` enforces the same rule at the database, so the check-then-delete race fails safe |
...
| `/programmes/[id]` | The **HoD of the programme's department** edits PO/PSOs (`programme.manage`) — there is no programme coordinator, and one HoD covers every programme of the department. **Batches — add, rename, delete — belong to the Controller of Examinations** (`batches.manage`; CR-3 supersedes CR-2, which had given them to the HoD): batches and rosters moved together, on the same reasoning CR-2 gave — whoever holds the register should hold the container. It is an action of its own, not a widening of `departments.manage`, so managing a batch never carries the power to create or destroy departments and programmes — a distinction that still matters for the HoD, though CR-5 has since given the COE both |
| `/courses/new` | **CR-3: the Controller of Examinations** creates courses, in any batch of any department. Faculty are optional here — staffing stays the HoD's (FR-4) |
| `/courses/[id]` | The catalogue entry — code, title, semester, credits and the **Laboratory** flag — is `course.details.write`, **the COE's alone**; everyone else reads it. The flag hands a practical paper's external examination to the department, and is refused while external marks exist, since that would transfer marks already recorded. Assigned faculty below it is **`course.staff` — HoD only**: faculty see the roster read-only. Assigning an account without the Faculty role is refused (it would grant nothing), and the last instructor cannot be removed |
...
| `/batches/[id]/roster` | Roster import from Excel/CSV with a full preview; roster list. **CR-3: importing is the COE's** (`roster.manage`) — the register moved with the batches. **Download a blank template** (`/api/batches/[id]/roster/template`, `roster.manage` — the same authority as the import it feeds): CSV with a BOM so Excel reads UTF-8, headers the importer actually matches, two example rows showing the with-email and without-email shapes, and `#` guidance lines the parser skips. **Blank, unlike the mark template, which is deliberately pre-filled** — roster import only ever creates, so there is nothing for an empty file to destroy, and a pre-filled one would put 4,000 names on somebody's laptop |
| `…/enrolment` | Draw a course's students from the batch roster — tick names, never type register numbers |
| `…/marks` → `…/marks/[id]` | Assessment picker → the mark entry grid + paste/upload. The picker also carries **all assessments in one file**: download a course-wide workbook (a sheet per assessment) and upload it back, previewed and applied all-or-nothing. **CR-3 makes "may I enter these?" a per-assessment question** — an *Entered by* column says whose each row is, and the workbook silently covers only the sheets you may write, naming the rest rather than reporting them as unmatched. The end-semester paper is the COE's on a theory course and the department's on a Laboratory one |
...
| `…/learning-outcome` | **CR-7: learning outcome by knowledge level**, one question paper at a time. The blueprint (share of the paper per level) needs no marks at all; the class figures and the per-student table need them. Per-student rows and the Excel download require `marks.read`, so a Dean or IQAC reader keeps the blueprint and the class view without the register numbers (NFR-10). No CO or PO figure depends on any of it |
| `…/attainment` | Computed attainment, prominent warnings, workflow controls, the review history (CR-4), and the full drill-down. Exports: **PDF** (`/api/courses/[id]/report`, the filed document, the only one with charts), **Word** (`…/report-docx`, CR-6 — the same report, editable, for the §4.5 action plan) and **Excel** (`…/export`). All three are guarded by `course.read` and audited with the format recorded |
...
#   coe@nmc.dev  (CR-3: the Controller of Examinations)
```
