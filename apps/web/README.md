...
| `…/assessments` | List + add (any shape/rule/group) + **adopt template** + **clone setup** + save-as-template (HoD). A **Maximum** column shows each assessment's total, derived from its items (there is no `maxMark` column on `Assessment`), honouring §3.1 "answer any n of m" — where a section caps how many questions count, the obtainable maximum is shown against the printed total |
